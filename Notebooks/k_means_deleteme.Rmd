...
<pre><pre>$$W(C_k) = \Sigma_{x_i\in C_k}(x_i-\mu_k)^2$$</pre></pre>

 - <pre>$x_i$</pre> is an observation assigned to cluster <pre>$C_k$</pre>
 - <pre>$\mu_k$</pre> is the mean value of all observations assigned to cluster <pre>$C_k$</pre> (i.e, the centroid)
...
<pre><pre>$$d_{euc}(x,y) = \sqrt{\Sigma_{i=1}^{n}(x_i-y_i)^2}$$</pre></pre>

 - <pre>$n$</pre> is the number of observations
 - <pre>$y_i$</pre> is the value of the centroid
...
<pre><pre>$$totalWCSS = \Sigma^k_{k=1}W(C_k) =\Sigma^k_{k=1}\Sigma_{x_i\in C_k}(x_i-\mu_k)^2$$</pre></pre>
