...
We take the right-singular vector $V$, which contains a set of orthonormal vectors that spans the row space of our term-document matrix. By plotting the first two columns, we should observe a separation between the documents. 
```{r}
...
ggplot(df, aes(x = LSA_1, y = LSA_2, color = title )) + 
  ...
  coord_fixed()
```
We observe that the math-related documents are in the top-right quadrant, while the fantasy-related documents occupy the bottom-right quadrant. The sports-related documents occupy the bottom-left quadrant but are more spread. We can take the top terms by ordering the terms in each column within the left-singular vector $U$, which are orthnonormal vectors that span the column space of the term-document matrix. 
```{r}
